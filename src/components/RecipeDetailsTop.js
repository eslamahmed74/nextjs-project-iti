                        ...
                        <div className="relative group">
                            ...
                            <div className="absolute top-4 right-4 flex space-x-2">
                                <button
                                    ...
                                    className={`p-3 rounded-full backdrop-blur-md transition-all duration-300 ${isFavorited
                            ...
                            <div className="flex items-center space-x-1 mb-6">
                                <div className="flex items-center">
                                    {[...Array(5)].map((_, i) => (
                                        <Star
                                            key={i}
                                            className={`h-5 w-5 ${i < Math.floor(recipe.rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                                        />
                                    ))}
                                </div>
                                <span className="text-lg font-semibold text-gray-900 ml-2">{recipe.rating}</span>
                                <span className="text-gray-600">({recipe.reviewCount} reviews)</span>
                            </div>

                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8">
                                <InfoCard icon={<Clock />} title="Prep Time" value={`${recipe.prepTimeMinutes}m`} color="blue" />
                                <InfoCard icon={<Clock />} title="Cook Time" value={`${recipe.cookTimeMinutes}m`} color="green" />
                                <InfoCard icon={<Users />} title="Servings" value={recipe.servings} color="purple" />
                                <InfoCard icon={<ChefHat />} title="Calories" value={recipe.caloriesPerServing} color="orange" />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="bg-gradient-to-r from-orange-100 to-red-100 rounded-2xl p-6 mt-8">
                            <div className="grid sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <h3 className="font-semibold text-gray-900 mb-2">Meal Type</h3>
